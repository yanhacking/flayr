...
class ChatsScreensController extends CupertinoController {
  ...
  bool get isNewMessage => (chats + roomChats).any((chat) => (chat.newMsgCount ?? 0) > 0 || chat.newMsgCount == -1);
  ...
  void onChange(String keyword) {
    filterChats = chats.where((element) {
      return element.title?.toLowerCase().contains(keyword.toLowerCase()) ?? false;
    }).toList();
  ...
  void fetchChats() {
    chatsListener = db.collection(FirebaseConst.users).doc(myUser?.firebaseId() ?? '').collection(FirebaseConst.userList).where(FirebaseConst.isDeleted, isEqualTo: false).snapshots().listen((event) {
      chats.clear();
    ...
    roomChatsListener = db.collection(FirebaseConst.chats).orderBy('time', descending: true).where('usersIds', arrayContains: SessionManager.shared.getUserID()).snapshots().listen((event) {
      roomChats.clear();
      for (var element in event.docs) {
      ...
      ConfirmationSheet(
        ...
        onTap: () {
          ...
          if (chatUserRoom.type == 2) {
            ...
            map['${myUser?.id}'] = date;
            db.collection(FirebaseConst.chats).doc(chatUserRoom.conversationId).update({FirebaseConst.deleteChatIds: map});
          } else {
            db.collection(FirebaseConst.users).doc(myUser?.firebaseId()).collection(FirebaseConst.userList).doc('${chatUserRoom.userIdOrRoomId}').update({
              FirebaseConst.deletedId: date,
              FirebaseConst.lastMsg: '',
            });
          }
        },
      ),
      ...
      ConfirmationSheet(
        ...
        onTap: () {
          ...
          if (chatUserRoom.type == 2) {
            ...
            db.collection(FirebaseConst.users).doc(myUser?.firebaseId()).collection(FirebaseConst.userList).doc('${chatUserRoom.userIdOrRoomId}').update({FirebaseConst.deletedId: date, FirebaseConst.isDeleted: true});
          }
        },
      ),
    );
