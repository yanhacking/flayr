      ...
      Get.bottomSheet(ConfirmationSheet(
        ...
        onTap: () {
          ...
          UserService.shared.blockUser(user?.id ?? 0, () {
            ...
            FirebaseFirestore.instance.collection(FirebaseConst.users).doc(user?.firebaseId()).collection(FirebaseConst.userList).doc(myUser?.firebaseId()).update(othersMap);
            var myMap = {FirebaseConst.iBlocked: true};
            FirebaseFirestore.instance.collection(FirebaseConst.users).doc(myUser?.firebaseId()).collection(FirebaseConst.userList).doc(user?.firebaseId()).update(myMap);
            completion();
          });
        },
      ));
