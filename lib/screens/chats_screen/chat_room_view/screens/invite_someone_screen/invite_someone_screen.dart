...
class RoomMemberCard extends StatelessWidget {
  ...
  const RoomMemberCard({Key? key, required this.type, required this.user, this.onTapInvitation, this.onRequestAccept, this.onRequestReject, this.onAdminRemove, this.onMakeAdmin, this.onRemoveFromRoom, this.room}) : super(key: key);
  ...
  Widget button() {
    if (type == RoomMemberType.user) {
      ...
      return room?.getUserAccessType() == GroupUserAccessType.admin
          ? GestureDetector(
              onTap: onAdminRemove,
              child: Container(
                padding: const EdgeInsets.symmetric(vertical: 7, horizontal: 18),
