...
class ChatUserRoom {
  ...
  ChatUserRoom({String? conversationId, bool? iAmBlocked, bool? iBlocked, String? deletedId, bool? isDeleted, bool? isMute, String? lastMsg, int? newMsgCount, DateTime? time, String? title, String? profileImage, num? userIdOrRoomId, List<num>? usersIds, int? type, Map<String, String>? deleteChatIds, Map<String, num>? unreadCounts}) {
    _title = title;
  ...
  Map<String, dynamic> toFireStore() {
    return {if (conversationId != null) "conversationId": _conversationId, if (iAmBlocked != null) "iAmBlocked": _iAmBlocked, if (iBlocked != null) "iBlocked": _iBlocked, if (deletedId != null) "deletedId": _deletedId, if (isDeleted != null) "isDeleted": _isDeleted, if (isMute != null) "isMute": _isMute, if (lastMsg != null) "lastMsg": _lastMsg, if (newMsgCount != null) "newMsgCount": _newMsgCount, if (time != null) "time": _time, if (type != null) "type": _type, if (title != null) "title": _title, if (profileImage != null) "profileImage": _profileImage, if (userIdOrRoomId != null) "userIdOrRoomId": _userIdOrRoomId, if (usersIds != null) "usersIds": _usersIds, if (deleteChatIds != null) "deleteChatIds": _deleteChatIds, if (unreadCounts != null) "unreadCounts": _unreadCounts};
  }
...
extension ChatUserExt on User {
  ChatUser toChatUser() {
    return ChatUser(fullName: fullName, image: profile, isVerified: isVerified == 1, userid: id?.toInt(), username: username);
  }

  AudioSpaceUser toAudioSpaceUser(AudioSpaceUserType type) {
    print("AUDIO_SPACE_USER: ${this.toJson()}");
    return AudioSpaceUser(
      isVerified: isVerified == 2 || isVerified == 3,
      fullName: fullName,
      ...
      image: profile,
      type: type,
      micStatus: AudioSpaceUserType.admin == type ? AudioSpaceMicStatus.on : AudioSpaceMicStatus.muted,
    );
  }
}
