...
class XMarkButton extends StatelessWidget {
  ...
  const XMarkButton({Key? key, this.color}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: CircleAvatar(
        radius: 15,
        backgroundColor: (color ?? cWhite).withValues(alpha: 0.1),
        foregroundColor: (color ?? cWhite).withValues(alpha: 0.5),
        child: const Icon(
          Icons.close_rounded,
          size: 18,
        ),
...
class VerifyIcon extends StatelessWidget {
  const VerifyIcon({this.user, Key? key, this.isPlaceholder = false}) : super(key: key);
  final User? user;
